...
class Utils(object):
    ...
    def write_read_event(self, string, event_count, event, device_string, device_count):
        ...
        if string is not None:
            ...
            if event.view is not None:
                f_read_trace.write(
                    str(event_count) + "::" + event.action + "::" + device_string + "::" + event.text + "::" + event.view.text + "::" + event.view.description + '\n')
            else:
    ...
    def write_event(self, event, device_count, f_trace, activity=None, run_count=None):
        if event.view is not None:
            f_trace.write(str(event.event_count) + "::" + event.action + "::device" + str(self.devices[device_count].device_num) + "::" + event.text + "::" + event.view.line)
            f_trace.write(str(event.event_count) + "::" + event.action + "::device" + str(self.devices[0].device_num) + "::" + event.text + "::" + event.view.line)
            # print("event_view")
            # print("activity", activity)
            # 判断从write_event是否传来activity，现在只在executor.py的start函数中传来activity，如果activity没有传来，则在本函数中获取
            if activity is None:
                _activity = self.devices[0].use.app_current()['activity']
            else:
                _activity = activity
            event_trace = {
                'event_action': event.action,
                'event_id': int(event.event_count),
                "event_info": str(event.event_count) + "::" + event.action + "::" + event.text,
                "run_count": run_count or "",
                'view_info': {
                    'activity': _activity,
                    'className': event.view.className,
                    'index': event.view.index,
                    'text': event.view.text,
                    'resourceId': event.view.resourceId,
                    'package': event.view.package,
                    'description': event.view.description,
                    'checkable': event.view.checkable,
                    'clickable': event.view.clickable,
                    'enabled': event.view.enabled,
                    'focusable': event.view.focusable,
                    'focused': event.view.focused,
                    'scrollable': event.view.scrollable,
                    'longClickable': event.view.longClickable,
                    'password': event.view.password,
                    'selected': event.view.selected
                },
                'bounds': event.view.bounds,
                'time': datetime.datetime.now().strftime("%H:%M:%S")
            }
            print(event_trace)
            with open(self.out_json, 'a', encoding='utf-8') as fw:
                json.dump(event_trace, fw, indent=4, ensure_ascii=False)
                fw.write(',\n')
        else:
            f_trace.write(str(event.event_count) + "::" + event.action + "::device" + str(self.devices[device_count].device_num) + "::" + event.text + "::None" + '\n')
    ...
    def generate_outline_html(self, output_path, strategy_list):
        ...
        tip = "<div style=\"text-align: center\">\n" \
              "<h2>" \
              "This is a bug page that runs the TestDroid tool" \
              "</h2>\n" \
