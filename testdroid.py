...
class TestDroid(object):
    ...
    def __init__(self,
        ...
        self.logger = logging.getLogger('TestDroid')
        self.enabled = True
        TestDroid.instance = self
        self.timer = None
    ...
    @staticmethod
    def get_instance():
        if TestDroid.instance is None:
            print("Error: TestDroid is not initiated!")
            sys.exit(-1)
        return TestDroid.instance
    ...
    def start(self):
        ...
        if not self.enabled:
            return
        # self.logger.info("Starting TestDroid")
