...
class RandomPolicy(Policy):
    ...
    def choice_event(self, device, event_count):
        # 先把activity保存下来，传参给util.py
        activity = device.use.app_current()['activity']

        event_type = random.randint(0, self.pro_all - 1)
        event = None
        click_classname_lists = ["android.widget.RadioButton", "android.view.View", "android.widget.ImageView", "android.widget.View",
                                 "android.widget.CheckBox", "android.widget.Button", "android.widget.Switch", "android.widget.ImageButton",
                                 "android.widget.TextView", "android.widget.CheckedTextView", "android.widget.TableRow",
                                 "android.widget.EditText", "android.support.v7.widget.ar", "android.widget.LinearLayout",
                                 "android.support.v7.widget.LinearLayoutCompat", "android.widget.CheckedTextView",
                                 ]
        # 6145 "androidx.appcompat.widget.LinearLayoutCompat"
        click_package_lists = [self.app.package_name, "android", "com.android.settings", "com.google.android",
                               "com.google.android.inputmethod.latin", "com.google.android.permissioncontroller", "com.android.packageinstaller",
                               "com.android.permissioncontroller", "com.google.android.packageinstaller"]
        not_click_lists = ["com.ichi2.anki:id/action_sync", "com.android.packageinstaller:id/permission_deny_button",
                           "com.ichi2.anki:id/buttonDefaultNegative", "com.ichi2.anki:id/add_deck_action"]
        # 第一个是设置 第四个是普通对话框的cancel
        ...
        if event_type < self.pro_click:
            ...
            for view in device.state.all_views:
                ...
                #         views.append(view)
                # elif view.className == "android.widget.CheckBox" and view.checkable == "true" and view.package in click_package_lists:
                #     views.append(view)
                if view.resourceId == "com.ichi2.anki:id/DeckPickerHoriz":
                    views.append(view)
                if view.clickable != 'false' and view.className not in not_click_lists and view.resourceId not in not_click_lists and view.className in click_classname_lists and view.package in click_package_lists:
                    views.append(view)
            if len(views) > 0:
                ...
                # print("pro_click_re_choice")
                event, activity = self.choice_event(device, event_count)
        elif event_type < self.pro_longclick:
            # print("longclick")
            if device.use(longClickable=True).count < 1:
                # print("longclick_re_choice")
                event, activity = self.choice_event(device, event_count)
            else:
                views = []
                for view in device.state.all_views:
                    if view.className not in not_click_lists and view.resourceId not in not_click_lists and view.className in click_classname_lists and view.package in click_package_lists and (
                            view.longClickable == "true" or view.clickable == "true"):
                        views.append(view)
                if len(views) > 0:
                    event_view_num = random.randint(0, len(views) - 1)
                    event_view = views[event_view_num]
            ...
            if device.use(scrollable=True).count < 1:
                ...
                if len(views) > 0:
                    ...
                    direction_list = ["backward", "forward"]
            ...
            if device.use(className="android.widget.EditText").count < 1:
                # print("pro_edit_re_choice")
                event, activity = self.choice_event(device, event_count)
            else:
                views = []
                for view in device.state.all_views:
                    if view.clickable != 'false' and view.className == "android.widget.EditText":
                        views.append(view)
                if len(views) > 0:
                    event_view_num = random.randint(0, len(views) - 1)
                    event_view = views[event_view_num]
