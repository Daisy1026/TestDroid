...
class Executor(object):
    ...
    def execute_event(self, device, event, num):
        try:
            ...
            if num == 0:
                print(ex)
                print("kqc- execute_event another time")
            ...
            if "Start::" in line:
                ...
                if record_flag == True:
                    ...
                    self.utils.generate_html(
                        os.path.join(
                            self.error_path, linelist[1]), os.path.join(
    ...
    def start(self, strategy):
        # if execute serial, init the strategy of device1, otherwise, init all
        # the guest devices' strategies
        ...
        while run_count < self.testcase_count:
            ...
            while event_count < self.event_num:
                print("kqc- executor_event_count", event_count)
                # if the state of any device is different from last state,
                # stoat need to judge (loading, foreground, same) again
                change_flag = False
                ...
                if self.devices[0].last_state is not None and change_flag:
                    ...
                    if not self.devices[0].state.same(self.devices[1].state):
                        ...
                        if self.checker.check_setting_request():
                            event = Event(
                                None, "check_setting_request", self.devices[1], event_count)
                ...
                try:
                    event, activity = self.policy.choice_event(self.devices[0], event_count)
                ...
                # device0 can not execute choice event, choice another event,
                # skip this loop
                if not success and fail_device == 0:
                    # 基本不会出现
                    print("kqc- not success and fail_device")
                    self.utils.print_dividing_line(False, event_count)
                    continue
                # other devices cannot execute choice event due to setting,
                # record fail, skip this loop
                elif not success:
                    print("kqc- not success")
                    self.utils.print_dividing_line(False, event_count)
                    self.utils.write_read_event(None, event_count, event, "different", fail_device)
                    self.utils.write_event(event, fail_device, self.devices[fail_device].f_trace, activity, run_count)
                    self.utils.draw_event(event)
                    if not self.checkduplicate():
                        print("kqc-write error")
                        self.utils.draw_error_frame()
                        self.utils.write_error(
                            fail_device,
                            run_count,
                ...
                # all devices execute success, record event and update state
                    ...
                    self.utils.write_event(event, device.device_num, device.f_trace, activity, run_count)
                    event_count = self.save_all_state(event_count)
