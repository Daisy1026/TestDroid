...
class Checker(object):
    ...
    def __init__(self, devices, app, strategy_list, emulator_path, android_system, root_path, resource_path, testcase_count, event_num, timeout, setting_random_denominator,
                 rest_interval, choice):
    ...
    def check_start(self, times, strategy):
        try:
            if self.app.package_name == "fr.free.nrw.commons" and times == 0:
                print("Feimao start 0--rest_interval * 20")
    ...
    def check_loading(self):
        ...
        for device in self.devices:
            if device.use(className="android.widget.ProgressBar").count > 0 and wait_time < self.rest_interval * 5:
                time.sleep(self.rest_interval * 1)
                wait_time = wait_time + self.rest_interval * 1
                print("wait load")
